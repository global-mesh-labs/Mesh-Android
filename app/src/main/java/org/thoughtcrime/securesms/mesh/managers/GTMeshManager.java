...
public class GTMeshManager implements GTCommandCenter.GTMessageListener, GTCommand.GTCommandResponseListener, GTErrorListener
{
    ...
    public void initToken(Context context) {
        ...
        // example code the performs same operations that fails in static GTKeyPair generateKeyPair() in ECDHLibrary.java
        try
        {
            ...
            Log.w(TAG,String.valueOf(publicKeyBytes));
            Log.w(TAG,String.valueOf(privateKeyBytes));
        }
        catch (Exception e)
        {
            Log.w(TAG, e);
